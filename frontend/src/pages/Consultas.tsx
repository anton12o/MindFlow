...
const SortableItem = React.memo(function SortableItem({ item, tipos, selectedIds, toggleSelect }: SortableItemProps) {
  ...
  return (
    <li ref={setNodeRef} style={style}
      className={`bg-bg-secondary rounded-lg border p-3 transition-colors ${selectedIds.has(item.id) ? 'border-accent ring-1 ring-accent' : 'border-border hover:border-accent/50'} ${isDragging ? 'opacity-disabled shadow-[--elevation-2]' : ''}`}>
      <div className="flex items-center gap-2">
        ...
        <input type="checkbox" checked={selectedIds.has(item.id)}
          onChange={() => toggleSelect(item.id)} className="accent-accent" aria-label={`Selecionar ${item.titulo}`} />
...
export default function Consultas() {
  ...
  const [tempView, setTempView] = useState<string | null>(() => localStorage.getItem('consultas_tempView'))
  const scrollRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    if (tempView) {
      localStorage.setItem('consultas_tempView', tempView)
    } else {
      localStorage.removeItem('consultas_tempView')
    }
  }, [tempView])
      ...
      <div className="w-72 border-r border-border p-4 shrink-0 flex flex-col h-full overflow-y-auto">
        ...
        <form onSubmit={e => { e.preventDefault(); setQueryFormError(''); setQueryGroupError('')
          ...
          <button type="submit" disabled={createMut.isPending}
            className="px-3 py-1.5 bg-accent text-accent-foreground text-sm rounded-lg transition-all active:scale-95 disabled:opacity-disabled">{createMut.isPending ? 'Criando...' : 'Criar'}</button>
        </form>
          ...
          {!qLoad && !qErr && (queries || []).map(q => (
            <div key={q.id} className="flex items-center gap-1">
              ...
              <button onClick={() => setConfirmDeleteId(q.id)}
                className="text-xs text-text-muted hover:text-danger" aria-label={`Excluir consulta ${q.nome}`}><X size={12} /></button>
            </div>
          ))}
        </div>
      </div>
              ...
              {selectedIds.size > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => { if (!batchField) { notify('Selecione um campo'); return }; batchMut.mutate() }}
                    ...
                    className="px-3 py-1 bg-accent text-accent-foreground text-xs rounded-lg transition-all active:scale-95 disabled:opacity-disabled">
              ...
              <div className="flex gap-4 overflow-x-auto pb-4" style={{ minHeight: '60vh' }}>
                {(() => {
                  ...
                  return [...ordem, ...extra].map(col => (
                      <div key={col} className="bg-bg-secondary rounded-xl border border-border w-56 flex-shrink-0 flex flex-col">
                  ...
                  {(result?.dados || []).map(item => {
                    ...
                    return (
                      <div key={card.id} onClick={() => toggleSelect(card.id)}
                        className={`group bg-bg-secondary rounded-xl border border-border overflow-hidden transition-all hover:scale-[1.02] hover:shadow-[--elevation-2] ${selectedIds.has(card.id) ? 'border-accent ring-2 ring-accent' : 'border-border'}`}>
