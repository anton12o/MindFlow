...
vi.mock('../store/pomodoro', async (importOriginal) => {
  const actual = await importOriginal()
  return { ...actual, usePomodoroContext: () => ({ state: { ativo: mockPomodoroAtivo }, dispatch: vi.fn(), config: {} as any, setConfig: vi.fn(), resetTimer: vi.fn(), advancePhase: vi.fn(), startedAtRef: { current: 0 }, audioCtxRef: { current: null }, saveHeartbeat: vi.fn(), clearHeartbeat: vi.fn() }) }
})
