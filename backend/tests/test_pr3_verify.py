...
class TestPomodoroHabito:
    def test_finalizar_sessao_cria_registro_no_habito_especifico(self, client, session):
    ...
    def test_finalizar_sessao_sem_contexto_nao_cria_registro_extra(self, client, session):
        ...
        foco = session.exec(select(Habito).where(Habito.nome == "Foco")).first()
        registros = session.exec(select(RegistroHabito)).all()
...
class TestAutoCreateWikilink:
    def test_create_from_wikilink_endpoint(self, client):
    ...
    def test_create_from_wikilink_duplicado_rejeita(self, client):
        client.post("/api/notas/from-wikilink", json={"titulo": "Unica"})
        resp = client.post("/api/notas/from-wikilink", json={"titulo": "Unica"})
        assert resp.status_code == 409

    def test_create_from_wikilink_titulo_vazio_rejeita(self, client):
        resp = client.post("/api/notas/from-wikilink", json={"titulo": ""})
        assert resp.status_code == 400

    def test_create_from_wikilink_titulo_grande_rejeita(self, client):
...
class TestCascadeDeletes:
    def test_delete_habito_cascade_registros(self, client, session):
    ...
    def test_delete_nota_com_flashcard_set_null(self, client):
    ...
    def test_create_from_wikilink_com_titulo_comum(self, client):
    ...
    def test_create_from_wikilink_com_espacos(self, client):
        """Whitespace trimming"""
