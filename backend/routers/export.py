...
def _generate_portability_pack() -> str:
    ...
    try:
        ...
        logger.warning("Limpeza temp dir falhou: %s", e)
    return str(zip_path)
...
def export_all(_rl: None = Depends(export_limiter), session: Session = Depends(get_session)):
    try:
        ...
        raise HTTPException(500, "Erro ao exportar dados")
...
def export_csv(_rl: None = Depends(export_limiter), session: Session = Depends(get_session)):
    try:
        ...
        raise HTTPException(500, "Erro ao exportar CSV")
...
def export_tarefas_feitas(
    ...
    try:
        ...
        logger.error("Erro ao exportar tarefas feitas: %s", e)
        raise HTTPException(500, "Erro ao exportar tarefas feitas")
